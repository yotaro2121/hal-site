...
1. デスクトップの **「HAL作品管理.command」をダブルクリック**(編集画面が Chrome で自動で開く。フォルダ選択は不要)
2. 写真をドラッグして追加/差し替え(トップの壺=HERO・WORKS・COLOR DNA すべて対応)、作品名・色・表示位置を編集、↑↓で並べ替え
3. 「保存」→「公開」を押す → 1〜2分で公開URLに反映
...
- `assets/` — サイトで使用する作品写真。JPEG（原本・フォールバック）と、配信用の WebP を同名で対にして置きます（下の「画像の配信方式」参照）。
...
モバイルの読み込みが重かったため、**JPEG は残したまま**、表示は WebP を優先する構成にしています。

- `works-data.js` / `site-text.js` に書くパスは **`.jpg` のまま**。変更不要。
- index.html は表示時に `<picture>` を組み立て、**同名の `.webp`** を `<source>` に、元の `.jpg` を `<img>` のフォールバックに置く。
  WebP 非対応ブラウザや `.webp` が無い場合は自動で JPEG に戻る（`picGuard()`）。
- **`.jpg` と `.webp` は必ず対で置く**こと。片方だけ差し替えると古い画像が表示される。
  管理画面（admin.html）から差し替えれば **WebP は自動生成される**ので、通常は意識不要。
- ヒーローだけは幅違いを持ち、`srcset` で端末に応じて出し分ける:
  `hero-2x-800.webp` / `hero-2x-1200.webp` / `hero-2x.webp`(1600w) ＋ フォールバック `hero-2x.jpg`。
  admin.html のヒーロー差し替えはこの3枚をまとめて書き出す。
- `assets/thumbs/` — COLOR DNA のチップ（64px 表示）用の 192px サムネ。
  原寸5枚で 866KB あったものを 24KB に落とすためのもので、DNA セクションが画面に入るまで読み込まない。
- `og-image.jpg` と `hal-turn-poster.jpg` は WebP 化しない（前者は SNS クローラ互換、後者は `<video poster>` で形式の出し分けができないため）。

手作業で画像を追加した場合は、同じ場所に `.webp` を作っておくこと（長辺基準ではなく**横幅 1600px 上限・品質 80** で生成）。

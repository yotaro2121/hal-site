...
仕組み: .command が `admin-server.py`(ローカル専用サーバー、127.0.0.1:8765)を起動し、admin.html がサーバーモードで動く。保存は `/api/save`・`/api/image`、公開は `/api/publish`(git add/commit/push)。従来の「フォルダを開く」+`公開.command` も残っているので、サーバーが動かないときの予備手段として使える。
